...
export const CreatePatientModal: React.FC<CreatePatientModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!telefono.trim() || telefono.trim() === '+58') {
    ...
    if (initialPdfFile && initialPdfDataUrl) {
      documents.push({
        ...
        patientId,
        title: initialPdfTitle || initialPdfFile.name,
    ...
    const newPatient: PatientRecord = {
      ...
      email: email.trim() || `${nombre.toLowerCase().trim()}.${apellido.toLowerCase().trim()}@correo.com`,
      ...
      alergias: alergias.trim() || 'Ninguna conocida (NKDA)',
      ...
      clinicalNotes: clinicalNotes.trim() || 'Paciente recién incorporado al expediente clínico de EQUILIBRA.',
      documents,
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">
                Diagnóstico / Motivo de Consulta Inicial *
              </label>
              <textarea
                rows={2}
                required
